...
type MockDeployer struct {
	...
	PushOnDemandSkillsFn        func(ctx context.Context, workerName, runtime string, skills []string, remoteSkills []v1beta1.RemoteSkillSource) error
...
func (m *MockDeployer) PushOnDemandSkills(ctx context.Context, workerName, runtime string, skills []string, remoteSkills []v1beta1.RemoteSkillSource) error {
	m.mu.Lock()
	m.Calls.PushOnDemandSkills = append(m.Calls.PushOnDemandSkills, workerName)
	fn := m.PushOnDemandSkillsFn
	m.mu.Unlock()
	if fn != nil {
		return fn(ctx, workerName, runtime, skills, remoteSkills)
	}
	return nil
}
