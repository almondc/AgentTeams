...
type MockManagerDeployer struct {
	...
	PushOnDemandSkillsFn  func(ctx context.Context, name, runtime string, skills []string, remoteSkills []v1beta1.RemoteSkillSource) error
...
func (m *MockManagerDeployer) PushOnDemandSkills(ctx context.Context, name, runtime string, skills []string, remoteSkills []v1beta1.RemoteSkillSource) error {
	m.mu.Lock()
	m.Calls.PushOnDemandSkills = append(m.Calls.PushOnDemandSkills, name)
	fn := m.PushOnDemandSkillsFn
	m.mu.Unlock()
	if fn != nil {
		return fn(ctx, name, runtime, skills, remoteSkills)
	}
	return nil
}
